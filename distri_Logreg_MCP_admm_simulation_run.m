...
for i=1:m
    ...
    [beta history] = distr_MCP_logreg(A0, Y0, lambda(i), N, 1.0, 1.0);   % (X, Y, lambda, N, rho, alpha)
    beta_path(:,i)=beta; 
    i
end

[opt,Mse]=CV_distri_MCP_logistic(A0, Y0, lambda, N, beta_path, beta_int, beta_zero);
beta=beta_path(:,opt);

%% without Cross validation
% lambda = 0.1 * 1/(n*N) * norm((1-ratio)*sum(A0(Y0==1,:),1) + ratio*sum(A0(Y0==-1,:),1), 'inf');
% [beta history] = distr_MCP_logreg(A0, Y0, lambda, N, 1.0, 1.0);    % (X, Y, lambda, N, rho, alpha)
